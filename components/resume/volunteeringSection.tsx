...
function VolunteeringSection() {
  const Volunteer : Volunteer[] = [
    ...
    },
    {
      ...
      society: "FIT Moments, IT Faculty Official Media Unit, University of Moratuwa",
    },
    {
    ...
    <div className='grid grid-rows-4 gap-6 pt-12'>
      {Volunteer.map((item, index) => (
        <div key={index} className='flex gap-5 rounded-xl'>
          <div>
          <FaDotCircle className='text-fontSecondary mt-2' />
          </div>
          <div>
          <div className='flex gap-2'>
            <div className='text-2xl '>{item.position}</div>
          </div>
          <div className='text-fontGray text-sm mb-2'>{item.time}</div>
